	...
	if p.idex2.Valid && !memStall && !execStall {
		...
		if p.exLatency2 == 0 {
			...
			nextEXMEM2 = SecondaryEXMEMRegister{
				...
				ALUResult:  execResult.ALUResult,
				StoreValue:  execResult.StoreValue,
				StoreValue2: func() uint64 { if p.idex2.Inst == nil || p.idex2.Inst.Op != insts.OpSTP { return 0 }; rt2 := p.idex2.Inst.Rt2; return p.forwardFromAllSlots(rt2, p.regFile.ReadReg(rt2)) }(),
				Rd:         p.idex2.Rd,
...
func (p *Pipeline) tickQuadIssue() {
	...
	if p.idex3.Valid && !memStall && !execStall {
		...
		if p.exLatency3 == 0 {
			...
			nextEXMEM3 = TertiaryEXMEMRegister{
				...
				ALUResult:  execResult.ALUResult,
				StoreValue:  execResult.StoreValue,
				StoreValue2: func() uint64 { if p.idex3.Inst == nil || p.idex3.Inst.Op != insts.OpSTP { return 0 }; rt2 := p.idex3.Inst.Rt2; return p.forwardFromAllSlots(rt2, p.regFile.ReadReg(rt2)) }(),
				Rd:         p.idex3.Rd,
	...
	if p.idex4.Valid && !memStall && !execStall {
		...
		if p.exLatency4 == 0 {
			...
			nextEXMEM4 = QuaternaryEXMEMRegister{
				...
				ALUResult:  execResult.ALUResult,
				StoreValue:  execResult.StoreValue,
				StoreValue2: func() uint64 { if p.idex4.Inst == nil || p.idex4.Inst.Op != insts.OpSTP { return 0 }; rt2 := p.idex4.Inst.Rt2; return p.forwardFromAllSlots(rt2, p.regFile.ReadReg(rt2)) }(),
				Rd:         p.idex4.Rd,
	...
	// Detect load-use hazards for primary decode.
	// Also check secondary and tertiary slots (idex2, idex3) for loads that
	// could create hazards with the next instruction group.
	loadUseHazard := false
	if p.ifid.Valid {
		nextInst := p.decodeStage.decoder.Decode(p.ifid.InstructionWord)
		if nextInst != nil && nextInst.Op != insts.OpUnknown {
			...
			// Check secondary slot (idex2) — LDP may be in secondary position
			if !loadUseHazard && p.idex2.Valid && p.idex2.MemRead && p.idex2.Rd != 31 {
				loadUseHazard = p.hazardUnit.DetectLoadUseHazardDecoded(
					p.idex2.Rd, nextInst.Rn, sourceRm, usesRn, usesRm)
				if !loadUseHazard && p.idex2.Inst != nil && p.idex2.Inst.Op == insts.OpLDP {
					loadUseHazard = p.hazardUnit.DetectLoadUseHazardLDPRt2(
						p.idex2.Inst.Rt2, nextInst.Rn, sourceRm, usesRn, usesRm)
				}
			}
			// Check tertiary slot (idex3) — LDP may be in tertiary position
			if !loadUseHazard && p.idex3.Valid && p.idex3.MemRead && p.idex3.Rd != 31 {
				loadUseHazard = p.hazardUnit.DetectLoadUseHazardDecoded(
					p.idex3.Rd, nextInst.Rn, sourceRm, usesRn, usesRm)
				if !loadUseHazard && p.idex3.Inst != nil && p.idex3.Inst.Op == insts.OpLDP {
					loadUseHazard = p.hazardUnit.DetectLoadUseHazardLDPRt2(
						p.idex3.Inst.Rt2, nextInst.Rn, sourceRm, usesRn, usesRm)
				}
			}
		}
	}
...
func (p *Pipeline) forwardFromAllSlots(reg uint8, currentValue uint64) uint64 {
	...
	if p.memwb.Valid && p.memwb.RegWrite {
		if p.memwb.Rd == reg {
			if p.memwb.MemToReg {
		...
		} else if p.memwb.MemToReg && p.memwb.Inst != nil && p.memwb.Inst.Op == insts.OpLDP {
			// LDP writes second register (Rt2) with second loaded value (MemData2)
			if p.memwb.Inst.Rt2 != 31 && p.memwb.Inst.Rt2 == reg {
				currentValue = p.memwb.MemData2
			}
		}
	}
	if p.memwb2.Valid && p.memwb2.RegWrite {
		if p.memwb2.Rd == reg {
		...
		} else if p.memwb2.MemToReg && p.memwb2.Inst != nil && p.memwb2.Inst.Op == insts.OpLDP {
			if p.memwb2.Inst.Rt2 != 31 && p.memwb2.Inst.Rt2 == reg {
				currentValue = p.memwb2.MemData2
			}
		}
	}
	if p.memwb3.Valid && p.memwb3.RegWrite {
		if p.memwb3.Rd == reg {
		...
		} else if p.memwb3.MemToReg && p.memwb3.Inst != nil && p.memwb3.Inst.Op == insts.OpLDP {
			if p.memwb3.Inst.Rt2 != 31 && p.memwb3.Inst.Rt2 == reg {
				currentValue = p.memwb3.MemData2
			}
		}
	}
	// Slots 4-8 are ALU-only (no memory port), so no LDP Rt2 forwarding needed.
	// LDP can only reach slots 1-3 (maxMemPorts=3). Use MemData for safety if MemToReg is true.
	...
	// NOTE: For load instructions (MemRead=true), ALUResult is the load address,
	// NOT the loaded data. Data is only available after MEM stage (in memwb).
	// We should NOT forward from exmem for loads — load-use hazard detection
	// should ensure no instruction accesses load results before they're in memwb.
	// For non-load ALU instructions, ALUResult is the computed value.
	if p.exmem.Valid && p.exmem.RegWrite && p.exmem.Rd == reg && !p.exmem.MemRead {
		currentValue = p.exmem.ALUResult
	}
	if p.exmem2.Valid && p.exmem2.RegWrite && p.exmem2.Rd == reg && !p.exmem2.MemRead {
		currentValue = p.exmem2.ALUResult
	}
	if p.exmem3.Valid && p.exmem3.RegWrite && p.exmem3.Rd == reg && !p.exmem3.MemRead {
		currentValue = p.exmem3.ALUResult
	}
	if p.exmem4.Valid && p.exmem4.RegWrite && p.exmem4.Rd == reg && !p.exmem4.MemRead {
		currentValue = p.exmem4.ALUResult
	}
	if p.exmem5.Valid && p.exmem5.RegWrite && p.exmem5.Rd == reg && !p.exmem5.MemRead {
		currentValue = p.exmem5.ALUResult
	}
	if p.exmem6.Valid && p.exmem6.RegWrite && p.exmem6.Rd == reg && !p.exmem6.MemRead {
		currentValue = p.exmem6.ALUResult
	}
	if p.exmem7.Valid && p.exmem7.RegWrite && p.exmem7.Rd == reg && !p.exmem7.MemRead {
		currentValue = p.exmem7.ALUResult
	}
	if p.exmem8.Valid && p.exmem8.RegWrite && p.exmem8.Rd == reg && !p.exmem8.MemRead {
	...
	if p.idex5.Valid && !memStall && !execStall {
		...
		if p.exLatency5 == 0 {
			...
			nextEXMEM5 = QuinaryEXMEMRegister{
				...
				ALUResult:  execResult.ALUResult,
				StoreValue:  execResult.StoreValue,
				StoreValue2: func() uint64 { if p.idex5.Inst == nil || p.idex5.Inst.Op != insts.OpSTP { return 0 }; rt2 := p.idex5.Inst.Rt2; return p.forwardFromAllSlots(rt2, p.regFile.ReadReg(rt2)) }(),
				Rd:         p.idex5.Rd,
	...
	if p.idex6.Valid && !memStall && !execStall {
		...
		if p.exLatency6 == 0 {
			...
			nextEXMEM6 = SenaryEXMEMRegister{
				...
				ALUResult:  execResult.ALUResult,
				StoreValue:  execResult.StoreValue,
				StoreValue2: func() uint64 { if p.idex6.Inst == nil || p.idex6.Inst.Op != insts.OpSTP { return 0 }; rt2 := p.idex6.Inst.Rt2; return p.forwardFromAllSlots(rt2, p.regFile.ReadReg(rt2)) }(),
				Rd:         p.idex6.Rd,
	...
	if p.idex7.Valid && !memStall && !execStall {
		...
		if p.exLatency7 == 0 {
			...
			nextEXMEM7 = SeptenaryEXMEMRegister{
				...
				ALUResult:  execResult.ALUResult,
				StoreValue:  execResult.StoreValue,
				StoreValue2: func() uint64 { if p.idex7.Inst == nil || p.idex7.Inst.Op != insts.OpSTP { return 0 }; rt2 := p.idex7.Inst.Rt2; return p.forwardFromAllSlots(rt2, p.regFile.ReadReg(rt2)) }(),
				Rd:         p.idex7.Rd,
	...
	if p.idex8.Valid && !memStall && !execStall {
		...
		if p.exLatency8 == 0 {
			...
			nextEXMEM8 = OctonaryEXMEMRegister{
				...
				ALUResult:  execResult.ALUResult,
				StoreValue:  execResult.StoreValue,
				StoreValue2: func() uint64 { if p.idex8.Inst == nil || p.idex8.Inst.Op != insts.OpSTP { return 0 }; rt2 := p.idex8.Inst.Rt2; return p.forwardFromAllSlots(rt2, p.regFile.ReadReg(rt2)) }(),
				Rd:         p.idex8.Rd,
